...
namespace GestureRecognizer.Platforms.Apple;
...
internal class MauiCameraView : UIView, IAVCaptureVideoDataOutputSampleBufferDelegate, IAVCaptureFileOutputRecordingDelegate, IAVCapturePhotoCaptureDelegate
{
    ...
    private void ProccessGesture()
    {
        MainThread.BeginInvokeOnMainThread(() =>
        {
            try
            {
                ...
                //cameraView.DecodeGesture(image2);
            }
            catch
            {
            }
    ...
    private void ProcessImage(CIImage capture)
    {
        new Task(() =>
        {
            ...
            if (cameraView.GestureDetectionEnabled && currentFrames >= Model.FrameInterval)
            {
                bool processGesture = false;
                lock (cameraView.currentThreadsLocker)
                {
                    if (cameraView.currentThreads < cameraView.GestureDetectionMaxThreads)
                    {
    ...
    public void DidOutputSampleBuffer(AVCaptureOutput captureOutput, CMSampleBuffer sampleBuffer, AVCaptureConnection connection)
    {
        frames++;
        currentFrames++;
        if (frames >= 12 || (cameraView.GestureDetectionEnabled && currentFrames >= Model.FrameInterval))
        {
